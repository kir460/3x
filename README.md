...
Корневой путь URL-адреса подписки меняем с sub на любой другой, я например изменю на TV65  /tv/
...
После активации подписки нас просят сменить url подписки на более 
сложный, переходим во вкладку “Подписка JSON” и придумываем более 
сложный “Корневой путь URL-адреса подписки”, я например 
укажу tv55led, сохраняем и перезапускаем.
...
# Поиск оптимального сервера в подсетях TLS 1.3 через TLS Scanner 
...
6) Введите в окне PowerShell строку ниже, вместо <ip address vps> подставьте свой IP адрес сервера, где будет 3x-ui
7) Аргумент > results.txt складывает в текстовый файл в папку, откуда мы возьмем результаты. Спустя 2-5 мин можем  прекращать поиск и перейти в тестовый файл results.txt
8) Меняем SNI на нужный, будьте внимательны к пунктуации. Старайтесь использовать более редкие варианты, и стараться избегать более популярных. 
